...
def render_time_progress(progress_info: str, total_info: str) -> None:
    ...
    PASSED_CHAR = "━"
    UNPASSED_CHAR = tcolor("─", styles=[AnsiStyle.DIM])
...
def render_metadata_line(title: str, author: str, year: str) -> None:
    ...
    if len(author) >= title_padding + len(author):
        content = tcolor(author, styles=[AnsiStyle.DIM, AnsiStyle.ITALIC])
        content += "   "
        content += title
        content += "   "
        content += tcolor(year, styles=[AnsiStyle.DIM, AnsiStyle.ITALIC])
